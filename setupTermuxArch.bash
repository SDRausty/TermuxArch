...
_CHKSELF_() {	# compare file setupTermuxArch and the file being used
	...
	then	# find and unset functions
		...
		# find variables
		UNVAR="$(grep '="' "${0##*/}"|grep -v -e \] -e ARGS -e TAMPDIR -e WDIR -e WFDIR -e INSTALLDIR -e ROOTDIR|grep -v +|sed 's/declare -a//g'|sed 's/declare//g'|sed 's/export//g'|sed -e "s/[[:space:]]\+//g"|cut -d"=" -f 1|sort -u)"
		# unset variables
		for UNSET in $UNVAR
		do
			unset "$UNSET"
