...
_CHKSELF_() {	# compare file setupTermuxArch and the file being used
	...
	then	# find and unset functions
		...
		# find variables
		UNVAR="$(grep '="' "${0##*/}"|grep -v -e \] -e ARGS -e CPUABI -e INSTALLDIR -e TAMPDIR -e VERSIONID -e WDIR|grep -v +|sed 's/declare -a//g'|sed 's/declare//g'|sed 's/export//g'|sed -e "s/[[:space:]]\+//g"|cut -d"=" -f 1|sort -u)"
		# unset variables
		for UNSET in $UNVAR
		do
			unset "$UNSET"
...
_INTRO_() {
	...
	then
		printf "\\n\\e[0;33m%s\\e[1;33m%s\\e[0;33m.\\e[0m\\n\\n" "TermuxArch WARNING!  " "The root directory structure of ~/${INSTALLDIR##*/} is correct; Cannot continue '${0##*/} install' to install Arch Linux in Termux PRoot!  See '${0##*/} h[elp]' and '$STARTBIN h[e[lp]]' for more information"
		exit 205
	fi
