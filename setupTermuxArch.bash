...
## File 'updrepoTermuxArch.bash' will execute 'git pull' and populate git repository modules, and file 'updrepoTermuxArch.bash' can also be run directly in a PRoot environment.  File 'updepoTermuxArch.bash's functions are not related to updating functions run by command 'setupTermuxArch r[e[fresh]]' that have completely different update functions.  The command 'setupTermuxArch r[e[fresh]]' attempts to refresh the Arch Linux in Termux PRoot installation and the TermuxArch generated scripts to the newest version.  It also helps in the installation and configuration process if everything did not go smoothly on the first try to install Arch Linux in Termux PRoot.
...
## These files 'setupTermuxArch[.{bash,bin,sh}]' will NOT selfupdate to the most recent version published if they are used inside their git repository;  In this case 'git pull' or 'updrepoTermuxArch.bash' can be employed to update to the newest published version.
