...
edq ()
{
	...
	read -p "Enable only one mirror that is the closest to you geographically. Choose a nearby mirror in the mirrors file. Do you want to use \`nano\` or \`vi\` to edit your Arch Linux configuration files [n|v]? "  nv
		fi
	if [[ $nv = [Nn]* ]];then
		ed=nano
