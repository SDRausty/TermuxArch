...
bloom () {
	...
	printf "\\e[1;34mTermuxArch Bloom option via \\e[1;32msetupTermuxArch.sh bloom\\e[0m 📲\\n\\n\\e[0m"'\\e]2;  Thank you for using TermuxArch Bloom option via `setupTermuxArch.sh bloom` 📲 \007'
	ls -agl
	printf "\\n"
	pwd
	printf "\\n"
...
bloomdependsblock () {
	intrbloom 
	cd "$HOME"/TermuxArchBloom
	printf "\\e[1;34mTermuxArch Bloom option via \\e[1;32mbash setupTermuxArch.sh run\\e[0m 📲\\n\\n\\e[0m"'\\e]2;  Thank you for using TermuxArch Bloom option via `bash setupTermuxArch.sh run` 📲 \007'
	ls -al
	printf "\\n"
	pwd
...
intro () {
	printf '\\e]2;  bash setupTermuxArch.sh 📲 \007'
	rmarchq
	rootdirexception 
	spaceinfo
...
intrbloom () {
	printf '\\e]2;  bash setupTermuxArch.sh bloom 📲 \007'
	spaceinfo
...
introdebug () {
	printf '\\e]2;  bash setupTermuxArch.sh sysinfo 📲 \007'
	spaceinfo
...
introrefresh () {
	printf '\\e]2;  bash setupTermuxArch.sh refresh 📲 \007'
	spaceinfo
...
manual () {
	printf '\\e]2;  Thank you for using `bash setupTermuxArch.sh manual` 📲 \007'
	editors
	if [ -f "setupTermuxArchConfigs.sh" ];then
...
pe () {
	printf "\\n\\e[1;31mPrerequisites exception.  Run the script again…\\n\\n\\e[0m"'\\e]2;  Thank you for using setupTermuxArch.sh.  Run `bash setupTermuxArch.sh` again…\007'
	exit
}
...
rootdirexception () {
	if [[ "$installdir" = "$HOME" ]] || [[ "$installdir" = "$HOME"/ ]] || [[ "$installdir" = "$HOME"/.. ]] || [[ "$installdir" = "$HOME"/../ ]] || [[ "$installdir" = "$HOME"/../.. ]] || [[ "$installdir" = "$HOME"/../../ ]];then
		printf "\\n\\e[1;31mRootdir exception.  Run the script again with different options…\\n\\n\\e[0m"'\\e]2;Rootdir exception.  Run `bash setupTermuxArch.sh` again with different options…\007'
		exit
	fi
}
...
unset LD_PRELOAD
versionid="gen.v1.6 id857705468"
