...
makesetupbin ()
{
	...
	if [[ "$kid" -eq 1 ]]; then
		cat >> root/bin/setupbin.sh <<- EOM
		exec proot --kill-on-exit --kernel-release=4.14.15 --link2symlink -0 -r $HOME$rootdir/ -b /dev/ -b /sys/ -b /proc/ -b /storage/ -b $HOME -w $HOME /bin/env -i HOME=/root TERM="$TERM" PS1='[termux@arch \W]\$ ' $HOME$rootdir/root/bin/finishsetup.sh 
		EOM
	else
		cat >> root/bin/setupbin.sh <<- EOM
		exec proot --kill-on-exit --link2symlink -0 -r $HOME$rootdir/ -b /dev/ -b /sys/ -b /proc/ -b /storage/ -b $HOME -w $HOME /bin/env -i HOME=/root TERM="$TERM" PS1='[termux@arch \W]\$ ' $HOME$rootdir/root/bin/finishsetup.sh 
		EOM
	fi
