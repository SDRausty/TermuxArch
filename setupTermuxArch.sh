...
depends ()
{
	if [ ! -e $PREFIX/bin/bsdtar ] || [ ! -e $PREFIX/bin/curl ] || [ ! -e $PREFIX/bin/proot ] || [ ! -e $PREFIX/bin/wget ] ; then
		printf "Checking prerequisites and upgrading Termux.\n\n\033[36;1m"
...
spaceinfo ()
{
	...
	if [[ $usrspace = *G ]] || [[ $usrspace = *T ]];then
		...
		spaceMessage="\n\n\033[1;33mTermuxArch: WARNING!  \033[36mStart thinking about cleaning out some stuff.  \033[1;33mThe user space on this device is less than $usrspace free space.  \033[1;36mThe recommended minimum to install Arch Linux in Termux PRoot is more than 1G of free user space.\n\033[0m"
	fi
	printf "$spaceMessage"
}
