...
introbloom ()
{
	...
	spaceinfo
	printf "\n\033[36;1m 🕛 < 🕛 \033[1;34mTermuxArch bloom option.  Run \033[36mbash setupTermuxArch.sh --help \033[34;1mfor additional information.  Ensure background data is not restricted.  If you do not see one o'clock 🕐 below, check the wireless connection.  "
}
...
{
	if [ -d $HOME/arch ] ;then
		printf "\n\033[0;33mTermuxArch: \033[1;33mDIRECTORY WARNING!  \`$HOME/arch/\` directory detected.  \033[1;30mTermux Arch installation will continue.  \033[0;33mInstalling into a clean directory is recommended.  \033[1;30mUninstalling before continuing is suggested.\n"
		rmarch
	fi
}
...
{
	if [ -d $HOME/TermuxArchBloom ] ;then
		printf "\n\n\033[0;33mTermuxArch: \033[1;33mDIRECTORY WARNING!  \`$HOME/TermuxArchBloom/\` directory detected.  \033[1;30mTermux Arch will continue.\n"
		rmbloom
	fi
}
...
spaceinfo ()
{
	...
	if [[ $usrspace = *G ]];then
		...
		if [ $(getprop ro.product.cpu.abi) = arm64-v8a ];then
			if [[ "$usspace" < "1.5" ]];then
				spaceMessage="\n\033[0;33mTermuxArch: \033[1;33mFREE SPACE WARNING!  \033[36mStart thinking about cleaning out some stuff.  \033[33m$usrspace of free user space is available on this device.  \033[1;36mThe recommended minimum to install Arch Linux in Termux PRoot for aarch64 is 1.5G of free user space.\n\033[0m"
			else
				spaceMessage=""
			fi
		elif [ $(getprop ro.product.cpu.abi) = armeabi-v7a ];then
			if [[ "$usspace" < "1.25" ]];then
				spaceMessage="\n\033[0;33mTermuxArch: \033[1;33mFREE SPACE WARNING!  \033[36mStart thinking about cleaning out some stuff.  \033[33m$usrspace of free user space is available on this device.  \033[1;36mThe recommended minimum to install Arch Linux in Termux PRoot for armv7 is 1.25G of free user space.\n\033[0m"
			else
				spaceMessage=""
			fi
		else
